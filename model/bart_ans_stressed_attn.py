...
def load_as_model():
    ...
    for block in origin_layers:
        if idx in plug_in_layers:
            block.encoder_attn = Ans_Stressed_Attention(            
                    block.encoder_attn.embed_dim,
                    block.encoder_attn.num_heads,
                    dropout=block.encoder_attn.dropout,
                    is_decoder=True,
            )
        idx += 1
    return model
